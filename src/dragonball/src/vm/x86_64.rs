...
use kvm_bindings::{
    kvm_enable_cap, kvm_irqchip, kvm_pit_config, kvm_pit_state2, KVMIO, KVM_CAP_EXIT_HYPERCALL,
    KVM_CAP_SPLIT_IRQCHIP, KVM_CAP_X86_APIC_BUS_CYCLES_NS, KVM_PIT_SPEAKER_DUMMY,
};
...
impl Vm {
    ...
    pub fn init_microvm(
        ...
        if self.firmware_type == Some(FirmwareType::Tdshim) {
            ...
            self.device_manager.vfio_manager.lock().unwrap().to_aml_bytes(&mut body_sink);
